...
│      └─ ...
└─ samples
   └─ Khaos.AppLifecycle.SampleWebApp
```

### 2.2 Assemblies & Namespaces

- **Assembly:** `Khaos.AppLifecycle.dll`
...
```text
Khaos.AppLifecycle
Khaos.AppLifecycle.Flows
Khaos.AppLifecycle.Scheduling
Khaos.AppLifecycle.Hosting
Khaos.AppLifecycle.Events
Khaos.AppLifecycle.Options
```

Folders in `src/Khaos.AppLifecycle` should roughly match these namespaces.
...
```csharp
namespace Khaos.AppLifecycle.Options;
...
```csharp
namespace Khaos.AppLifecycle.Events;
...
```csharp
namespace Khaos.AppLifecycle.Flows;
...
- **Unit tests** (in `Khaos.AppLifecycle.Tests`):
